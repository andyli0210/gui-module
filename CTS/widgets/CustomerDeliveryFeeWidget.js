IFL.CTS.CustomerDeliveryFeeWidget = function(_options, groupMember, isRoot) {
    var options = {
        treeWidth: 500
    };
    ...
    function init() {
        if (groupMember.members && groupMember.members.length > 0) {
            for (var m in groupMember.members) {
                ...
                var memberWidget = new IFL.CTS.CustomerDeliveryFeeWidget(options, member);
    ...
    function initTreeWidget(isLeaf) {
        feeInputBox = $('<input/>').attr('value', groupMember.deliveryFees).css('width', 50);
        
        feeInputBox.change(function() {
            groupMember.deliveryFees = parseFloat(feeInputBox.val());
        });
