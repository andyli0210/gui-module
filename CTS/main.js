function AppStarter() {
    ...
    function initAccountDetails() {
        ctsService.getAccountDetails(function(response) {
            ...
            if (response.accountDetails.hasGeoInput && response.accountDetails.hasOriData) {
                ...
                if (response.accountDetails.baseCaseConfirmed) {
                    initCtsLayout();
                } else {
                    initStatusPanel();
                    //init user data
    ...
    function initCtsOptPanel(customerGroups) {
        ...
        ctsOptPanel.registerHandler('showMap', function(optInfo) {
            ...
            window.open('mapView_standalone.html?optId=' + optInfo.id, mapViewName, 'fullscreen=yes');
        });
...
//        });
        ...
        //skip route optimisation, after confirm base case
        statusContainer.registerHandler('confirm', function() {
            ctsService.confirmOpt("baseCase", function(response) {
                initCtsLayout();
            });
