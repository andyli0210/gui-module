...
(function($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function setupCanvases() {
            ...
            existingTimeOverlay = placeholder.children("canvas.timeOverlay");
            ...
            if (existingCanvas.length == 0 || existingOverlay == 0) {
                ...
                timeOverlay = makeCanvas(false, "timeOverlay");
                ...
                timeOverlay = existingTimeOverlay.get(0);
            ...
            tctx = timeOverlay.getContext("2d");

            // we include the canvas in the event holder too, because IE 7
            // sometimes has trouble with the stacking order
            eventHolder = $([overlay, canvas, timeOverlay]);
            ...
            if (reused) {
                ...
                eventHolder.unbind();
                placeholder.children().not([canvas, overlay, timeOverlay]).remove();
            }
        ...
        function draw() {
            ...
            octx.clearRect(0, 0, canvasWidth, canvasHeight);
            //tctx.clearRect(0, 0, canvasWidth, canvasHeight);
            //plot.drawIflSeries(iflSeries);
            placeholder.trigger('draw');
        ...
        function insertAxisTitles() {
            ...
            var axes = plot.getAxes();
            var box = axes.xaxis.box;
            var titleHight = plotOffset.top + 25;
            var rowHeight = (canvasHeight - titleHight)/11;
        ...
        function triggerClickHoverEvent(eventname, event, seriesFilter) {
            ...
            //debugger;
            //var item = plot.findNearByIflSeries(canvasX, canvasY, plot.iflSeries);

            //placeholder.trigger(eventname, [ pos, item ]);
            //console.debug(canvasPos.x);
            placeholder.trigger(eventname, [pagePos, canvasPos]);
        }
        ...
        function unhighlightSeries(s) {
            var axisx = s.xaxis;
            var axisy = s.yaxis;
            
            var leftX = axisx.p2c(s.minX);
            var topY = axisy.p2c(s.maxY);
                
            var rightX = axisx.p2c(s.maxX);
            var bottomY = axisy.p2c(s.minY);
            
            octx.clearRect(0, 0, canvasWidth, canvasHeight);
        //octx.clearRect(leftX+50 ,topY-5, rightX-leftX+50, bottomY-topY+5);
        //debugger;
        //ctx.clearRect(leftX+50 ,topY, rightX-leftX+10, (topY-bottomY)+10);
            
        //triggerRedrawOverlay();
            
        }
        
        function highlightSeries(s) {
            var axisx = s.xaxis;
            var axisy = s.yaxis;
            
            //debugger;
            var leftX = Math.max(s.minX, axisx.min);
            leftX = axisx.p2c(leftX);
            
            var bottomY = Math.max(s.minY, axisy.min);
            bottomY = axisy.p2c(bottomY);
                
            var rightX = Math.min(s.maxX, axisx.max);
            rightX = axisx.p2c(rightX);
            
            var topY = Math.min(s.maxY, axisy.max);
            topY = axisy.p2c(topY);
                
            var context = octx;
            context.beginPath();
            // debugger;
            //var plotOffset = plot.getPlotOffet();
            context.rect(leftX+ plotOffset.left ,bottomY + plotOffset.top, rightX-leftX, topY-bottomY);
